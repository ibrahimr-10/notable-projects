...
// Fetch data for yellow cards per gameweek
$query = "SELECT gameweek, SUM(yellow_cards) AS total_yellow_cards FROM player_statistics GROUP BY gameweek ORDER BY gameweek";
...
if ($fire) {
    ...
    while ($result = mysqli_fetch_assoc($fire)) {
        $data[] = array($result['gameweek'], (int)$result['total_yellow_cards']);
    }
} else {
...
<head>
    ...
    <script type="text/javascript">
        ...
        function drawChart() {
            ...
            data.addColumn('number', 'Yellow Cards');     // Label for y-axis
            data.addRows(jsonData);

            // Set chart options
            var options = {
                title: 'Yellow Cards per Gameweek',
                curveType: 'function',
